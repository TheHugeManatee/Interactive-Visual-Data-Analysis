...
std::string GetExePathA()
{
	...
	if(0 == GetModuleFileNameA(nullptr, buffer, bufferSize))
	{
		return std::string("");
	}
...
HRESULT TransferFunctionEditor::onCreateDevice(ID3D11Device* pd3dDevice)
{
	...
	pTfLineAlpha_	= new TransferFunctionLine( XMFLOAT4(0,1,1,1), v2iSizeTfEdt_.x - 1, v2iSizeTfEdt_.y - 1);
	pTfLineR_ 		= new TransferFunctionLine( XMFLOAT4(1,0,0,1), v2iSizeTfEdt_.x - 1, v2iSizeTfEdt_.y - 1);
	pTfLineG_ 		= new TransferFunctionLine( XMFLOAT4(0,1,0,1), v2iSizeTfEdt_.x - 1, v2iSizeTfEdt_.y - 1);
	pTfLineB_ 		= new TransferFunctionLine( XMFLOAT4(0,0,1,1), v2iSizeTfEdt_.x - 1, v2iSizeTfEdt_.y - 1);
	...
	if( it == s_mapEffect_.end() || (*it).second == NULL )
	{
		...
		std::ifstream file(GetExePathA() + "TransferFunctionEditor.fxo", std::ios::in | std::ios::binary);
	...
	pFxSrHistogram_ = pEffect->GetVariableByName("txHistogramm_")->AsShaderResource();
...
void TransferFunctionEditor::onDestroyDevice()
{
	...
	if(pTfEdtUi_!=NULL) {
		...
		pTfEdtUi_ = nullptr; //bugfix: was producing "Not Found" errors
	}
...
void TransferFunctionEditor::onFrameRender( double fTime, float fElapsedTime )
{
...
void TransferFunctionEditor::drawTransferFunction( )
{
	...
	if(pFxSrTfTex_ && pTfSRV_ != NULL)
		pFxSrTfTex_->SetResource(pTfSRV_);
...
{	
	...
	for( int ch = 0; ch < 4; ch++)
	{
		...
		{
            XMFLOAT2 v(static_cast<float>(pCp->v2iPt_.x)/static_cast<float>(v2iSizeTfEdt_.x-1), static_cast<float>(pCp->v2iPt_.y)/static_cast<float>(v2iSizeTfEdt_.y-1));
			binary_file->write(reinterpret_cast<char *>(&(v)), sizeof(XMFLOAT2));
			pCp = pCp->pNext_;
		}
	}
...
void TransferFunctionEditor::loadTransferFunction(std::ifstream* binary_file)
{
	...
	for( int ch = 0; ch < 4; ch++ )
	{
		...
		binary_file->read(reinterpret_cast<char *>(&v),sizeof(XMFLOAT2));
