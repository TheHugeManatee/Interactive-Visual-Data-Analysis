...
TransferFunctionLine::TransferFunctionLine(XMFLOAT4 v4fLineColor, UINT uiMaxX, UINT uiMaxY) 
	: pVb_(NULL)
	, pVl_(NULL)
	, pFirstPoint_(NULL)
	, uiNumPoints_(0)
...
{
    pData_[index] = XMFLOAT2(static_cast<float>(pCp->v2iPt_.x)/static_cast<float>(v2iMaxXY_.x), static_cast<float>(pCp->v2iPt_.y)/static_cast<float>(v2iMaxXY_.y) );
}
...
int TransferFunctionLine::getControlPointId(Vec2i v2iPos)
{
	...
	{
		i++;
        XMVECTOR v2fDist = XMVectorSet(static_cast<float>(pCurr_point->v2iPt_.x - v2iPos.x), static_cast<float>(pCurr_point->v2iPt_.y - v2iPos.y), 0, 0);
        float fLen = ceil(XMVectorGetX(XMVector2Length(v2fDist)));
...
HRESULT TransferFunctionLine::createVertexBuffer()
{
    ...
    pData_ = new XMFLOAT2[ iNumElements ];

	UINT uiSize = sizeof(XMFLOAT2) * iNumElements;
...
{
	UINT  uiSize = sizeof(XMFLOAT2) * uiNumPoints_;
